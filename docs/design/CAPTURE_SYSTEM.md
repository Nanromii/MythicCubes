...
Camera, target highlight, thao tác hold–drag–release, animation, VFX và SFX của capture được đóng gói ở Phase 12 theo [Định hướng hình ảnh, giao diện và âm thanh](VISUAL_AUDIO_UI_DIRECTION.md), không để tới polish cuối.
...
Implementation hiện tại vẫn là 1v1: cluster chỉ quyết định số cá thể và vị trí spawn; `WildCreatureRecord` chưa có `spawnGroupId`; `EncounterRecord`, snapshot và UI chỉ giữ một `wildId`; wild chưa có level, rarity, evolution stage, species capture difficulty hoặc legendary classification; hai thiết bị hiện tại dùng công thức `baseChance + missingHealthBonus`, từ chối full HP và luôn bị tiêu thụ sau một lần ném hợp lệ, kể cả thất bại. Game đích thay đổi eligibility full HP nhưng tài liệu không giả vờ source đã migration.

Sau khi thiết kế được duyệt, implementation phải migration theo từng bước nhỏ, giữ server authority, regression Phase 2–4 và dữ liệu session hiện có. Không đổi tên hai thiết bị hiện tại thành bốn tên production trong lượt tài liệu này.
...
| Encounter | Tập sinh vật server đã khóa độc quyền cho một player tại một thời điểm. Encounter có thể chỉ chứa một phần cluster nếu thành viên khác không hợp lệ. |
...
4. Loại thành viên đã thuộc encounter khác, đang `Returning`, `Defeated`, `Despawned` hoặc không còn record hợp lệ.
5. Claim toàn bộ tập còn lại vào cùng `encounterId` và `targetUserId`; client không có remote khai báo membership.
6. Tạo `EncounterRecord.wildIds` từ đúng kết quả claim và replicate snapshot cho owner.

Membership là snapshot tại thời điểm claim. Wild spawn/respawn muộn không tự chen vào encounter đang chạy. Nếu một thành viên bị player khác claim trước, encounter mới vẫn được phép bắt đầu với các thành viên còn hợp lệ; nếu không còn thành viên nào thì không tạo encounter. Một wild chỉ được thuộc tối đa một encounter.

Claim và mutation membership phải nằm trong coordinator server, không yield giữa kiểm tra và ghi state. Thứ tự `spawnGroupId`, khoảng cách rồi `wildId` giúp kết quả deterministic khi hai player đi vào cluster gần như đồng thời; player claim thành công trước giữ các wild đó, player còn lại chỉ nhận phần chưa bị claim.
...
- Owner chết/rời server, không còn companion sống, hoặc owner vượt `disengageRange` so với đội/encounter trong điều kiện server định nghĩa sẽ kết thúc **toàn bộ encounter**.
- Khi kết thúc toàn bộ, server bỏ claim mọi thành viên còn `Engaging`, chuyển từng wild sang `Returning` và xóa các companion target. Companion còn sống lập tức quay về follow owner.
...
5. Target còn sống, `Engaging`, chưa reserved/captured/defeated và thuộc đúng `encounterId`/`targetUserId`.
...
9. Tính chance, clamp và random roll hoàn toàn trên server.
10. Commit transaction idempotent rồi mới phát presentation/result snapshot.
...
- Failure không cấp creature/reward và không làm reset các thành viên còn lại.
...
- Thành viên đã bị player khác claim hoặc không hợp lệ bị loại, các thành viên còn lại vẫn tạo encounter đúng.
...
- Hai client tranh cùng cluster không thể cùng sở hữu một wild hoặc cùng nhận capture result.
